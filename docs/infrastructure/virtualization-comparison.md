...
RADIUS + 監視基盤 + 運用サービス群をまとめて運用する前提での評価。
...
- **Docker**: マルチサービスではCompose管理が複雑化するため学習コストがやや上がる。運用自動化も弱い
- **k3s**: Helmチャートで監視スタック一括導入が容易。監視統合・運用自動化で高評価
- **Proxmox VE**: マルチサービス管理・監視統合で高評価。WebUIからLXC群を統合管理でき、pve-exporterで基盤自体の監視も容易
- **Hyper-V**: マルチサービス管理はVM個別操作が必要で低評価
