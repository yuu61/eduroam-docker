...
学校法人コンピュータ総合学園（神戸電子専門学校・神戸情報大学院大学）にeduroamを導入するための技術検証環境。
...
graph TD
    subgraph eduroam階層構造
        ...
        ETLR1 --- FLR["JP FLR<br>NII運用<br>flr1/flr2.eduroam.jp"]
        FLR --- RADIUS["機関RADIUSサーバー<br>(IdP + SP)<br>realm: kobedenshi.ac.jp / kic.ac.jp"]
    end

    style RADIUS fill:#4a90d9,color:#fff
```

**IdP（Identity Provider）**: 自機関ユーザー（@kobedenshi.ac.jp / @kic.ac.jp）の認証を担当
...
sequenceDiagram
    ...
    AP->>R: Access-Request<br/>(anonymous@kobedenshi.ac.jp or @kic.ac.jp)
    ...
    rect rgb(220, 255, 220)
        ...
        D->>R: Identity: user@realm<br/>Password: ****
        R->>R: suffix: realm抽出
...
sequenceDiagram
    ...
    AP->>R: Access-Request
    R->>R: realm判定:<br/>other-univ.ac.jp ≠ ローカルrealm
    R->>FLR: proxy転送
...
- **外部Identity**: `anonymous@<realm>` を使用し、実ユーザー名を秘匿（例: `anonymous@kobedenshi.ac.jp`, `anonymous@kic.ac.jp`）
...
graph LR
    subgraph "Docker Network: eduroam-net"
        FR["freeradius<br>FreeRADIUS 3.2.6<br>:1812/udp, :1813/udp"]
        OL["openldap<br>OpenLDAP<br>:389"]
        TC["test-client<br>radtest<br>eapol_test"]
    end

    FR -- "LDAP検索/bind" --> OL
    TC -- "RADIUS<br>Access-Request" --> FR
```
...
```mermaid
graph TD
    FR["FreeRADIUS<br>(VM/物理)<br>:1812/udp, :1813/udp<br>UPKIサーバ証明書"]
    GW["Google Workspace<br>Secure LDAP<br>ldap.google.com:636"]
    AP["AP群<br>SSID: eduroam"]
    FLR["JP FLR<br>eduroam.jp"]
    UPKI["NII UPKI<br>証明書発行サービス"]

    FR -- "LDAPS" --> GW
    AP -- "RADIUS" --> FR
    FR -- "proxy" --> FLR
    UPKI -. "サーバ証明書発行<br>(RSA 2048bit, 396日)" .-> FR
```
...
graph TD
    ...
    E_AUTHN -- "TTLSトンネル内の<br>認証リクエスト" --> I_AUTH
```
...
flowchart TD
    ...
    PARSE --> CHECK{realm判定}
    CHECK -- "@kobedenshi.ac.jp" --> LOCAL["ローカル認証<br>(inner-tunnel)"]
    CHECK -- "@kic.ac.jp" --> LOCAL
    CHECK -- "@other-univ.ac.jp" --> PROXY["FLR proxy転送<br>(Phase 4で有効化)"]
    CHECK -- "realm無し (NULL)" --> REJECT["拒否"]
```

> **複数realm対応**: 両ドメイン（kobedenshi.ac.jp / kic.ac.jp）ともGoogle Workspace運用が確認済み（MX/SPFレコードより）のため、同一のGoogle Workspace Secure LDAPエンドポイント経由で認証可能。1参加機関の下で複数realmを登録できるかはNIIへの確認事項。
...
```mermaid
graph TD
    subgraph "kobedenshi.ac.jp"
        ROOT_KD["dc=kobedenshi,dc=ac,dc=jp"]
        PEOPLE_KD["ou=people"]
        GROUPS_KD["ou=groups"]
        U1["uid=testuser1<br>mail: testuser1@kobedenshi.ac.jp"]
        U2["uid=testuser2<br>mail: testuser2@kobedenshi.ac.jp"]
        G1["cn=eduroam-users"]
    ...
    subgraph "kic.ac.jp"
        ROOT_KIC["dc=kic,dc=ac,dc=jp"]
        PEOPLE_KIC["ou=people"]
        GROUPS_KIC["ou=groups"]
        U3["uid=testuser1<br>mail: testuser1@kic.ac.jp"]
        G2["cn=eduroam-users"]
...
- **外部Identity**: `anonymous@kobedenshi.ac.jp` / `anonymous@kic.ac.jp`（実ユーザー名を秘匿）
- **内部Identity**: `user@kobedenshi.ac.jp` / `user@kic.ac.jp`（TLSトンネル内のみ）
...
| 対象FQDN | `radius.kobedenshi.ac.jp` / `radius.kic.ac.jp`（要決定。1台で両realm処理） |
...
```mermaid
graph LR
    P1["Phase 1<br>ローカル検証<br>OpenLDAP<br>自己署名証明書"]
    P2["Phase 2<br>Google LDAP接続<br>クライアント証明書"]
    P3["Phase 3<br>Proxy検証<br>モックFLR"]
    P4["Phase 4<br>eduroam JP接続<br>NII登録<br>UPKIサーバ証明書"]
    P5["Phase 5<br>AP統合<br>SSID: eduroam<br>実機テスト"]
...
4. CSR作成（RSA 2048bit、CN=RADIUSサーバFQDN）→ TSVファイル作成 → アップロード（両ドメインで証明書が必要な場合はドメインごとにCSRを作成）
...
| 方法 | 内容 |
|------|------|
| メール認証 | `admin@<ドメイン>` 等の定型アドレスでランダム値を受信・返信 |
| DNSメール認証 | `_validation-contactemail.<ドメイン>` TXTレコードにメールアドレスを登録 |
| DNS認証 | `<ドメイン>` TXTレコードに認証局指定のランダム値を登録 |

> 両ドメイン（kobedenshi.ac.jp / kic.ac.jp）それぞれで審査が必要。DNS事業者が共通（dnsv.jp）のため、DNS認証・DNSメール認証は同一管理画面から設定可能。

> **DNS事業者**: 両ドメイン（kic.ac.jp / kobedenshi.ac.jp）ともNS: 01〜04.dnsv.jp。DNS認証・DNSメール認証を選択する場合、dnsv.jpの管理画面からTXTレコードを追加する。メール認証の場合はGoogle Workspaceの管理コンソールで定型アドレス（admin@等）を受信できるよう設定が必要。
...
| 神戸電子側のAP管理がベンダー4社混在・管理者分散で統一的な設定変更が困難 | Phase 5遅延 | KIC先行導入で実績を作り、法人総務トップダウンで展開。詳細は `deployment-strategy.md` |
| 専門学校側の組織的合意形成が困難（L8リスク） | 導入全体の停滞 | 法人総務経由のトップダウン展開でボトムアップ合意形成をバイパス |
