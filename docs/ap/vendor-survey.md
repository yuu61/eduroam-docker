...
| WPA3-Enterprise | 対応 | 対応 | 対応 | 対応（Wi-Fi 6+） | 未確認 | Wi-Fi 6世代で対応 |
...
| WAPM-AX4 | Wi-Fi 6 (ax) | 未確認 | 128台 | — | 約29,742円 |
...
- 学内の既存APはWi-Fi 5世代が大半で、802.11k/v/r対応機種が極めて少ない
- ベンダーが棟によってバラバラで、同一棟内でも混在する例がある
- 詳細な機種リストは未入手

## 今後のアクション

1. 各棟のAP機種リストを入手し、WPA2-Enterprise対応を確認する
2. eduroamは既存APのWPA2-Enterprise対応で導入可能（AP更新は別タイムライン）
3. AP更新計画時に、ベンダー統一と802.11k/v/r対応を考慮する
4. 代理店への見積もり依頼（教育機関向け割引の確認）
5. PoEスイッチの既存対応状況を確認する
