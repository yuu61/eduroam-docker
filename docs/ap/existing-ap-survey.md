...
- セキュリティ上の問題: オープンネットワーク4件（SHOKUINSHITSU, WAVEMASTER-0, R1FWIFI, guest）、WPA+TKIP 2件、PSK `12345678` 1件
...
**注**: SHOKUINSHITSUのOUI `00:a0:de` は他のYAMAHA AP（`ac:44:f2`）とは異なり、より古い世代のYAMAHA機器を示す。MACフィルタリングによるアクセス制御が行われている可能性があるが、暗号化なしのためセキュリティ上の意味は限定的。詳細は「セキュリティ上の懸念事項」セクションを参照。

**推定機種**: OUI `ac:44:f2` はYAMAHA WLXシリーズ。802.11ac対応のものはWLX402/WLX313相当、802.11n のみのものはそれ以前の世代の可能性。

**YAMAHA WLXシリーズの制限**: マルチキャスト→ユニキャスト変換機能、ND proxy機能ともに非対応。L2ブリッジとして動作し、IPv6マルチキャスト（NDP等）はそのままブロードキャストとして無線に送出される。マルチキャスト制御は上流スイッチ側に完全に依存するが、上流スイッチ（SWX2100/2110/2200世代）もMLDスヌーピング非対応の場合、IPv6マルチキャストは無制御のまま全ポート・全無線クライアントにフラッディングされる。
...
WLCによる自動チャネル割当がないため、各APが個別にチャネル設定されており干渉が発生している。

- 2.4 GHz帯で非干渉チャネル（1, 6, 11）以外の使用を確認: Buffalo APがch2を使用（ch1とch6の双方に干渉）
- 同一WLC配下にないベンダー混在APが同一エリアに密集しており、チャネル重複が避けられない構造
- 全棟のサーベイでチャネル干渉の実態を定量的に把握する必要がある

WLC環境では自動チャネル管理（Aruba ARM、Cisco RRM等）により隣接AP間のチャネル重複を動的に回避できるが、現環境ではこの機能が一切存在しない。

#### セキュリティ

| SSID | 問題 | リスク |
|------|------|--------|
| **SHOKUINSHITSU** (YAMAHA) | オープン/暗号化なし | 後述。職員室関連ネットワークの可能性があるが詳細不明 |
| **WAVEMASTER-0** (ICOM) | オープン/暗号化なし | 用途不明。暗号化なしのため盗聴・不正接続が容易 |

#### SHOKUINSHITSU の詳細

SSID名から「職員室」に関連するネットワークと推測されるが、不明な点が多い。

| 項目 | 確認済み事実 |
|------|------------|
| SSID | SHOKUINSHITSU |
| BSSID | 00:a0:de:98:7f:a8 |
| OUI | 00:a0:de = YAMAHA（WLXシリーズとは別のOUI。古い世代の機器の可能性） |
| 暗号化 | **なし（オープン）** |
| 無線規格 | 802.11n (Wi-Fi 4) |
| バンド | 2.4 GHz |
| 接続試行結果 | 接続が拒否された — MACアドレスフィルタリングが設定されている可能性 |

**観測された事実と考察**:

- OUI `00:a0:de` は `ac:44:f2`（WLXシリーズ）とは異なり、より古い世代のYAMAHA無線機器を示唆する
- 同じく職員室用途と思われる `MainStaffRoom`（OUI: ac:44:f2、WPA3-Personal）が別に存在しており、SHOKUINSHITSUとの関係性（同一エリアの新旧併存？別の職員室？）は不明
- 暗号化なしのオープンSSIDだが、実際に接続を試みた際に拒否された。MACアドレスフィルタリングによるアクセス制御と推定される
- 仮にMACフィルタリングで制御しているとすれば、暗号化なしのため正規端末のMACアドレスはプローブフレーム等で平文送出されており、フィルタリングの実効性はない
- 暗号化されていないため、仮に接続できた場合（あるいは正規端末の通信）はすべて平文で傍受可能

**残る疑問**:
- なぜ暗号化なしで運用しているのか（設定ミス？ 古い機器の制約？ 意図的？）
- MainStaffRoomとの使い分けは何か
- どのフロア・建物で使用されているか（北野校舎4Fからの検出だが、設置場所は不明）
- 接続拒否の正確な原因（MACフィルタ以外の可能性もある）
...
| KD-IT-testing | IT科テスト用（3棟複数フロアで確認） | WPA2/3-Personal | 後述 |
...
### KD-IT-testing: 建物横断SSIDの存在

`KD-IT-testing` SSIDが**3つの建物の複数フロア**にまたがって配信されていることを確認。

- **Buffalo AP** (18:c2:bf:61:fb:40) と **ICOM AP** (00:90:c7:02:34:ab/7b) の少なくとも2ベンダーから同一SSIDを確認済み
- ベンダー混在（Buffalo + ICOM）のためWLC管理ではなく、各APに個別設定されている可能性が高い
- 学内ネットワークは少なくとも2年以上（おそらく4年近く）構成変更がない
- SSIDの用途・意図は不明（「テスト用」「パケットキャプチャ付き」等の推測はあるが根拠なし。年間通じて常時配信されている）

**eduroam導入への示唆**: 建物横断で同一SSIDを展開した実績があること自体は事実。この展開の経緯と主導者の特定が、既存インフラの理解と協力者の確保に繋がる可能性がある。
...
| **チャネル干渉** | WLCなしで自動チャネル管理がなく、非干渉チャネル（1,6,11）外の使用やベンダー間のチャネル重複が発生。スループット低下・接続不安定の原因 |
